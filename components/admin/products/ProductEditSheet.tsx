...
import { DollarSign, ImageIcon, LayoutGrid, Loader2, Package, Tag } from "lucide-react"
...
export default function ProductEditSheet({ product, open, onOpenChange }: ProductEditSheetProps) {
  ...
  async function onInfoSubmit(values: InfoValues) {
    ...
    try {
      ...
      if (values.productName !== fetched.productName) changed.productName = values.productName
      if ((values.description ?? "") !== (fetched.description ?? ""))
        changed.description = values.description
      if ((values.imageUrl ?? "") !== (fetched.imageUrl ?? "")) changed.imageUrl = values.imageUrl
  ...
  async function onPricingSubmit(values: PricingValues) {
    ...
    try {
      ...
      if (values.currentPrice !== fetched.currentPrice) changed.currentPrice = values.currentPrice
      if (values.quantity !== fetched.quantity) changed.quantity = values.quantity
  ...
  async function onClassificationSubmit(values: ClassificationValues) {
    ...
    try {
      ...
      if (values.categoryId !== String(fetched.categoryId)) changed.categoryId = values.categoryId
      if (values.supplierId !== String(fetched.supplierId)) changed.supplierId = values.supplierId
        ...
        <SheetHeader className="border-b px-6 py-5">
          ...
          {fetched && !fetchLoading ? (
            <SheetDescription className="flex flex-wrap items-center gap-2 text-xs">
              <span className="text-muted-foreground font-mono">#{fetched.id}</span>
              <Badge variant={fetched.status ? "default" : "secondary"} className="text-xs">
          ...
          {fetchLoading ? (
            ...
            <Tabs defaultValue="info" className="space-y-5">
                    ...
                    <Form {...infoForm}>
                      <form onSubmit={infoForm.handleSubmit(onInfoSubmit)} className="space-y-4">
                          ...
                          render={({ field }) => (
                            <FormItem className="flex items-center justify-between rounded-lg border p-3">
                              <div className="space-y-0.5">
                                <FormLabel className="text-sm">Trạng thái hiển thị</FormLabel>
                        ...
                        <div className="flex justify-end pt-1">
                          <Button
                            type="submit"
                            disabled={!infoForm.formState.isDirty || infoSubmitting}
                          >
              ...
              <TabsContent value="pricing">
                <Card>
                  <CardHeader className="pb-4">
                    <CardTitle className="text-sm font-medium">Giá bán & Tồn kho</CardTitle>
                    <CardDescription className="text-xs">
                    ...
                    <Form {...pricingForm}>
                      ...
                      >
                        ...
                        <div className="flex justify-end pt-1">
                          <Button
                            type="submit"
                            disabled={!pricingForm.formState.isDirty || pricingSubmitting}
                          >
                            {pricingSubmitting && <Loader2 className="mr-2 size-4 animate-spin" />}
                            Lưu thay đổi
                          </Button>
                        </div>
                      </form>
              ...
              <TabsContent value="classification">
                <Card>
                  <CardHeader className="pb-4">
                    <CardTitle className="text-sm font-medium">Phân loại sản phẩm</CardTitle>
                    <CardDescription className="text-xs">
                    ...
                    <Form {...classificationForm}>
                      ...
                      >
                        ...
                        <div className="flex justify-end pt-1">
                          <Button
                            type="submit"
                            disabled={!classificationForm.formState.isDirty || classSubmitting}
                          >
                            {classSubmitting && <Loader2 className="mr-2 size-4 animate-spin" />}
                            Lưu thay đổi
                          </Button>
                        </div>
                      </form>
