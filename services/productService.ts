...
export const productService = {
  ...
  createProduct: async (data: ProductCreateDto): Promise<AdminProduct> => {
    const response = await api.post<ApiResponse<AdminProduct>>("/products/create", data)
   ...
   */
  updateProduct: async (id: string, data: ProductCreateDto): Promise<AdminProduct> => {
    const response = await api.put<ApiResponse<AdminProduct>>(`/products/${id}`, data)
    return response.data.data
  },
